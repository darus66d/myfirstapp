import 'package:flutter/material.dart';
import 'package:google_fonts/google_fonts.dart';
import 'package:myfirstapp/screen/product_screen.dart';

class CustomWidget extends StatelessWidget {
  const CustomWidget({super.key});

  @override
  Widget build(BuildContext context) {
    return Scaffold(
      body: Center(child: Column(
        mainAxisSize: MainAxisSize.min,
        children: [
          HomeCategoryWidget(title: 'Product',iconData: Icons.account_circle,widget: Icon(Icons.ac_unit_outlined),),
          HomeCategoryWidget(title: 'User',iconData: Icons.image,widget: Image.network("data:image/png;base64,",height: 26,),),
          HomeCategoryWidget(title: 'Information',iconData: Icons.inbox,widget: Text("DarusSalam"),),
          //HomeCategoryWidget(title: 'About',iconData: Icons.account_balance,),
          //HomeCategoryWidget(title: 'Contact',iconData: Icons.contact_mail,),





          ElevatedButton(onPressed: (){
            Navigator.push(context, MaterialPageRoute(builder: (Context)=> ProductScreen(title: 'From Routing Screen',)
            ));
            // Navigator.push(context, MaterialPageRoute(builder: (Context){return ProductScreen();}));
          }, child: Text("Go to next page"),),
          ElevatedButton(onPressed: (){
            Navigator.push(context, MaterialPageRoute(builder: (Context)=> ProductScreen(title: 'From Product Screen',)
            ));
            // Navigator.push(context, MaterialPageRoute(builder: (Context){return ProductScreen();}));
          }, child: Text("Go to next Screen"),),
        ],
      ),),
    );
  }
}

class HomeCategoryWidget extends StatelessWidget {
  final String title;
  final IconData iconData;
  final Widget widget;
  const HomeCategoryWidget({
    super.key, required this.title, required this.iconData, required this.widget,
  });

  @override
  Widget build(BuildContext context) {
    return SizedBox(
      height: 100,
      width: double.infinity,
      child: InkWell(
        onTap: (){},
        child: Card(
          color: Colors.amber.shade800,
          child:Center(child: Row(
            mainAxisAlignment: MainAxisAlignment.spaceAround,
            //mainAxisAlignment: MainAxisAlignment.spaceEvenly,
            //mainAxisAlignment: MainAxisAlignment.spaceBetween,
            children: [
              Icon(iconData,size: 18,),
              widget,
              Text(title,style: GoogleFonts.poppins(fontSize: 18,fontWeight: FontWeight.w900),),
              Icon(Icons.arrow_forward),
            ],
          )),
        ),
      ),
    );
  }
}



